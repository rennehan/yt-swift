...
import yt
from yt.frontends.pkdgrav.api import PkdgravDataset
from yt.frontends.pkdgrav.fields import metal_elements
...
g64 = "pkdgrav_64/output/snap_N64L16_135.hdf5"
gmhd = "pkdgrav_mhd_mwdisk/pkdgrav_mhd_mwdisk.hdf5"
gmhd_bbox = [[-400, 400]] * 3
...
def test_pkdgrav_64():
    ds = yt.load(g64)
    assert isinstance(ds, PkdgravDataset)
    for test in sph_answer(ds, "snap_N64L16_135", 524288, fields):
        test_pkdgrav_64.__name__ = test.description
        yield test


@requires_file(gmhd)
def test_pkdgrav_mhd():
    """
...
def test_gas_particle_fields():
    """
    Test fields set up in PkdgravFieldInfo.setup_gas_particle_fields.
    """
...
def test_star_particle_fields():
    """
    Test fields set up in PkdgravFieldInfo.setup_star_particle_fields.
    """
    ds = yt.load(gmhd, bounding_box=gmhd_bbox)
